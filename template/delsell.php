...
// echo "<script>alert(".$sell_id.")</script>";
...
$sql = " SELECT * FROM product where product_code = '" . $sell_id . "'";
  ...
  while ($row2 = $result2->fetch_assoc()) {
    ...
    if ($result29->num_rows > 0) {
      ...
      while ($row28 = $result29->fetch_assoc()) {
        ...
        $up = "UPDATE product SET quentity  =  '" . $row28['qui'] + $row2['quentity'] . "'  WHERE product_code = '" . $sell_id . "' ";
        $reup = $conn->query($up);
